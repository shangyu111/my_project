<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px 5%","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 0px)","fontSize":"14px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 20px 0","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 0px","borderColor":"#f6f6f6","display":"flex","justifyContent":"center","minHeight":"100px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","borderWidth":"2px 2px 0","fontSize":"inherit","borderStyle":"solid"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="科目名称" prop="kemumingcheng">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">科目名称</label>
						<el-select clearable v-model="searchForm.kemumingcheng" placeholder="请选择科目名称" >
							<el-option v-for="(item,index) in kemumingchengOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">教师姓名</label>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','签到')" @click="qiandaojiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","display":"none","height":"auto"}'></span>
								签到
							</el-button>
							<el-button class="btn8" v-if="isAuth('kechengxinxi','请假')" @click="qingjiajiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","display":"none","height":"auto"}'></span>
								请假
							</el-button>
							<el-button class="btn8" v-if="isAuth('kechengxinxi','添加考勤')" @click="xueshengkaoqinCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","display":"none","height":"auto"}'></span>
								添加考勤
							</el-button>
		...
		<qingjiajilu-cross-add-or-update v-if="qingjiajiluCrossAddOrUpdateFlag" :parent="this" ref="qingjiajiluCrossaddOrUpdate"></qingjiajilu-cross-add-or-update>
		<xueshengkaoqin-cross-add-or-update v-if="xueshengkaoqinCrossAddOrUpdateFlag" :parent="this" ref="xueshengkaoqinCrossaddOrUpdate"></xueshengkaoqin-cross-add-or-update>
...
import qingjiajiluCrossAddOrUpdate from "../qingjiajilu/add-or-update";
import xueshengkaoqinCrossAddOrUpdate from "../xueshengkaoqin/add-or-update";
	export default {
		data() {
			return {
				...
				qingjiajiluCrossAddOrUpdateFlag: false,
				xueshengkaoqinCrossAddOrUpdateFlag: false,
		...
		components: {
			...
			qingjiajiluCrossAddOrUpdate,
			xueshengkaoqinCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			qingjiajiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.qingjiajiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kechengxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.qingjiajiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.qingjiajiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			xueshengkaoqinCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.xueshengkaoqinCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kechengxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xueshengkaoqinCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xueshengkaoqinCrossaddOrUpdate.init(row.id,type);
				});
			},
