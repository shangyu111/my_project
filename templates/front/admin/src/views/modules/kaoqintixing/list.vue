			...
			<el-form class="center-form-pv" :style='{"padding":"20px 20px 0","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 0px","borderColor":"#f6f6f6","display":"flex","justifyContent":"center","minHeight":"100px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","borderWidth":"2px 2px 0","fontSize":"inherit","borderStyle":"solid"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">通知标题</label>
						<el-input v-model="searchForm.tongzhibiaoti" placeholder="通知标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">通知日期</label>
						<el-date-picker v-model="searchForm.tongzhiriqistart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="通知日期起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.tongzhiriqiend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="通知日期结束"></el-date-picker>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('kaoqintixing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('kaoqintixing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","borderColor":"#f6f6f6","margin":"0 0 0px","borderRadius":"0 0 4px 4px","background":"rgba(255,255,255,1)","borderWidth":"0 2px 2px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaoqintixing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('kaoqintixing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kaoqintixing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
	...
	kaoqintixingstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kaoqintixing/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
